...
extrato <- reactive( json2df(lista_extratos()[6]) %>%                      
...
DT::dataTableOutput("filtro_demonstrativo")
# tabela interativa. O usuário seleciona o relatório desejado ao clicar na tabela.
output$filtro_demonstrativo <- DT::renderDataTable(demonstrativos() %>% select(-dia,- cod_ibge,-tipo,-poder,-forma_envio, -mes, -tipo_relatorio),filter = 'top', server = TRUE)
# output$filtro_demonstrativo <- DT::renderDataTable(output$filtro_demonstrativo(),filter = 'top', server = TRUE)
...
dataTableOutput("teste")

output$teste <- renderDataTable(                        json2df(lista_extratos()[6]))
...
DT::dataTableOutput('filtro_matriz')
# tabela interativa. O usuário seleciona o relatório desejado ao clicar na tabela.
output$filtro_matriz <- DT::renderDataTable(matrizes() %>% filter (poder == "E")%>% select(exercicio, populacao, entregavel, periodo, periodicidade, data_status, fechamento),filter = 'top', server = TRUE)
...
# dados_api_matriz <- reactive(relatorios(c(matriz_filtrada()$classe_1,matriz_filtrada()$classe_2)))

# dados_api_matriz <- reactive(relatorios(c(matriz_filtrada()[,])))

dados_api_matriz <-reactive(relatorios(map_dfr(matriz_filtrada()[1:1], cbind.data.frame)))
