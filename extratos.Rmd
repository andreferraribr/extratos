...
extratos_msc<-reactive( relatorios (lista_extratos()) %>%
...
# criar link para API
url_api_msc<-reactive (extratos_msc()%>%
...
### 1.1) Clique para selecionar os relatórios desejados

```{r}

dataTableOutput('filtro_msc')

output$filtro_msc <- renderDataTable(url_api_msc() %>% select(-dia,- cod_ibge,-poder,-forma_envio),filter = 'top', server = TRUE)
...
teste_url <- reactive (url_api_relatorios() %>% select(url_api) %>% slice (input$filtro_rows_selected))
